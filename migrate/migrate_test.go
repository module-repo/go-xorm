...
var (
	migrations = []*Migration{
		{
			ID: "201608301400",
			Migrate: func(tx *go_xorm.Engine) error {
				return tx.Sync2(&Person{})
			},
			Rollback: func(tx *go_xorm.Engine) error {
...
func TestMigration(t *testing.T) {
	...
	db, err := go_xorm.NewEngine("sqlite3", dbName)
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func TestInitSchema(t *testing.T) {
	...
	m := New(db, DefaultOptions, migrations)
	m.InitSchema(func(tx *go_xorm.Engine) error {
...
func tableCount(db *go_xorm.Engine, tableName string) (count int) {
