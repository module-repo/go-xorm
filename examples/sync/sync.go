...
import (
	...
	"github.com/module-repo/go-xorm"
)
...
func sync(engine *go_xorm.Engine) error {
	return engine.Sync(&SyncLoginInfo2{}, &SyncUser2{})
}

func sqliteEngine() (*go_xorm.Engine, error) {
	f := "sync.db"
	//os.Remove(f)

	return go_xorm.NewEngine("sqlite3", f)
}

func mysqlEngine() (*go_xorm.Engine, error) {
	return go_xorm.NewEngine("mysql", "root:@/test?charset=utf8")
}

func postgresEngine() (*go_xorm.Engine, error) {
	return go_xorm.NewEngine("postgres", "dbname=xorm_test sslmode=disable")
}

type engineFunc func() (*go_xorm.Engine, error)
